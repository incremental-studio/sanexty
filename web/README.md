...
- `FABRIC_PREVIEW_SECRET` can be any random string (but avoid spaces), like `MY_SECRET` - this is used for [the Preview Mode](https://nextjs.org/docs/advanced-features/preview-mode).
- `FABRIC_PROJECT_ID`: Get the `projectId` value from the `sanity.json` file created in step 2.
- `FABRIC_API_TOKEN`: Copy the API token generated in the previous step.

Your `.env` file should look like this:

```bash
FABRIC_PREVIEW_SECRET=...
FABRIC_PROJECT_ID=...
FABRIC_API_TOKEN=...
```
...
```js
const previewSecret = 'MY_SECRET' // Copy the string you used for FABRIC_PREVIEW_SECRET
...
```
now secrets add FABRIC_preview_secret <FABRIC_PREVIEW_SECRET>
now secrets add FABRIC_api_token <FABRIC_API_TOKEN>
now secrets add FABRIC_project_id <FABRIC_PROJECT_ID>
```
